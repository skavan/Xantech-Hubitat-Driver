/*
child driver fo Xantech 8 Zone audio
...
Original for Monoprice 6 Zone amplifier. Authored by jorge.martinez
...
*/
metadata {
    definition(name: 'Child Xantech 8 Zone Amp Controller', namespace: 'suresh.kavan', author: 'Suresh Kavan') {
        capability 'Switch'
...
def UpdateData (NewData) {
    ...
    switch (statusType) {
        case 'PR':
            def power = NewData.substring(4, 5)
            if (power.toInteger()) {
                state.switch = 'on'
                state.status = 'on'
            ...
            break
        case 'MU':
            def mute = NewData.substring(4, 5)
            if (mute.toInteger()) {
                state.mute = 'muted'
        ...
        case 'VO':
            ...
            def endIndex = NewData.indexOf('+')
            // 3. Extract the substring using calculated indexes:
            def vol = NewData.substring(startIndex, endIndex)
            ...
            break
        case 'SS':
            def source = NewData.substring(4, 5)
        ...
        case 'BA':
            ...
            def startIndex = 4  // You know this will always be the 4th character (0-based)
            // 2. Find the index of the "+" character:
            def endIndex = msg.indexOf('+')
            // 3. Extract the substring using calculated indexes:
            def Balance = msg.substring(startIndex, endIndex)
        ...
        case 'BS':
            ...
            def endIndex = msg.indexOf('+')
            // 3. Extract the substring using calculated indexes:
            def Bass = msg.substring(startIndex, endIndex)
            if (state.bass != Bass) {
        ...
        case 'TR':
            ...
            def endIndex = msg.indexOf('+')
            // 3. Extract the substring using calculated indexes:
            def Treble = msg.substring(startIndex, endIndex)
...
def updated() {
    log.info('Xantech 8 Zone Amp Controller: updated()')
    initialize()
}
def pollSchedule() {
//    forcePoll()
...
def initialize() {
    log.info('Xantech 8 Zone Amp Controller: initialize() ' + state.name + state.zone)
}
...
def Source1() {
    ...
    try {
        ...
        state.trackDescription = "Source: ${state.mediaSource}"
        sendEvent(name: 'trackDescription', value: state.trackDescription, isStateChange: true)
        parent.sendMsg("!${state.zone}SS${state.mediaSource}+")
    } catch (Exception e) {
        log.warn "Call to off failed: ${e.message}"
    }
}
